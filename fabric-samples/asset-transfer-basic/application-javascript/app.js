...
async function registerListener(){
    try {
        ...
        const listener = await network.addBlockListener(async (event) => {
            ...
            console.log("transaction data incomming:");
            for(let i=0;i<event.getTransactionEvents().length;i++){

                const transaction = event.getTransactionEvents()[i];
                console.log(transaction);
                for(let j=0;i<transaction.getContractEvents().length;j++){
                    if(transaction.getContractEvents()[j] != null){
                        console.log(transaction.getContractEvents()[j]);
                    }
                }
            }
            console.log("listener custom " + counter++);
            testServer();
        },{});
...
async function communicateWithNetwork(response) {
    try {
